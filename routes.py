             ...
             try:
                ...
                if doc == "SleepDiary_m":
                    ...
                    for i in range(0,anz):
                        ...
                        datum = str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Day "])+"_"+str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Month "])+"_"+str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Year "])
                        filename = "SleepDiary_m"
                        Algo.csvExport(db.SleepDiary_m.find({"user":name_patient})[i],datum,filename) 
                        Algo.excelExport(db.SleepDiary_m.find({"user":name_patient})[i],datum,filename)
                ...
                if doc == "SleepDiary_e":
                    ...
                    for i in range(0,anz):
                        ...
                        datum = str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Day "])+"_"+str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Month "])+"_"+str(db.SleepDiary_e.find({"user":name_patient})[i]["Date"]["Year "])
                        filename = "SleepDiary_e"
                        Algo.csvExport(db.SleepDiary_e.find({"user":name_patient})[i],datum,filename)
                        Algo.excelExport(db.SleepDiary_e.find({"user":name_patient})[i],datum,filename) 
                ...
                if  doc == "PSQI":
                     ...
                     Algo.csvExport(db.PSQI.find_one({"username":name_patient},"PSQI","PSQI"))
                     Algo.excelExport(db.PSQI.find_one({"username":name_patient}),datum,filename)
...
def sleep_r():
    if request.method == "POST":
        ...
        """
        print("Test")
        mean_sleepeff,newTime2Bed = calc_sleep_re(time)

        flash("Your Sleepefficiency is: "+str(round(mean_sleepeff,2))+"%")
        flash("You should go to Bed at: "+str(newTime2Bed))
...
def calc_sleep_re(UserTime):
    ...
    if db.sleep_re.count_documents({"user":username_g}) < 1:
        #1 acess to this route or counter is 2 or above
        #setup a new timespace for the restriction
        db.sleep_re.insert_one({"username":username_g,
                               "Star_Date":{"Day": datetime.now().day,"Month": datetime.now().month,"Year": datetime.now().year},
                               "End_Date": {"Day": (datetime.now().day-7),"Month": datetime.now().month,"Year": datetime.now().year},
                               "counter":0})
        
        

    counter =  db.sleep_re.find_one({"username":username_g})["counter"]


    for i in range(0,7):
        calc.append(Algo.clcSleepTime(sleepDiary_m_day[i]["TimeLightOff[HH:MM]"], sleepDiary_m_day[i]["WakeUpTime[HH:MM]"],
        sleepDiary_m_day[i]["HowLongTotal[HH:MM]"],sleepDiary_m_day[i]["LightOff2Sleep[HH:MM]"]))
        calc2.append(Algo.clcSER(calc[i], sleepDiary_e_day[i]["Time2Bed"] ,sleepDiary_m_day[i]["RiseTime[HH:MM]"]))
        sleepTime = Algo.EffControl(calc[i],calc2[i],counter,username_g)
        newTime2Bed = Algo.setSleepTime(sleepTime, UserTime)
    ...
    if mean_sleepeff > 85:
            counter+= 1
            db.sleep_re.update_one({"user":username_g},{ "$set": {"counter":counter}})
    
    return mean_sleepeff,newTime2Bed
