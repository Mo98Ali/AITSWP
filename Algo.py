...
#############################################################
########## calculat Sleep Efficiency for Restriction ########
#############################################################
...
#####################################################
############ Cal the Go to Bed Time #################
#####################################################
def setSleepTime(SH, wakeUpTime):
    format = '%H:%M'
...
###################### Eff control ##########################
##Controll the Sleep Efficiency and increase the sleep Time##
#############################################################
def EffControl(sleepTime, sleepEff,counter,username_g):
    ...
    if sleepEff > 85 and counter>2:
        ...
        db.sleep_re.update({"user":username_g, '$set':{"Sleep_counter":0}})
...
def csvExport(data, identifier, filename):
    path = "./" + identifier +"_"+filename + ".csv"

    with open(path, 'a', newline='') as csvfile:  # Use 'a' mode for appending
...
def excelExport(data,identifier,filename):
    data_ = [data]
    df1 = pd.DataFrame(data_)

    path = r"./" +identifier+filename+ "export.xlsx"
